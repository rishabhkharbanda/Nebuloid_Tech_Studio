          ...
          <SectionReveal delay={0.05}>
            <Link href={`/insights/${featured.slug}`} className="group block">
              <motion.article
                ...
                className="grid cursor-pointer gap-8 border-b border-white/10 py-12 md:grid-cols-12 md:items-center md:gap-12 md:py-16"
              >
                {featured.image ? (
                ...
                <div className={featured.image ? 'md:col-span-7' : 'md:col-span-12'}>
              ...
              <SectionReveal key={post.slug || post.title} delay={0.1 + index * 0.06}>
                <Link href={`/insights/${post.slug}`} className="block">
                  <motion.article
                    whileHover={{ x: 4 }}
                    transition={{ duration: 0.4, ease: [0.2, 0.65, 0.3, 0.9] }}
                    className="group grid cursor-pointer gap-6 py-10 md:grid-cols-12 md:items-center md:gap-10 md:py-12"
                  >
                    {post.image ? (
