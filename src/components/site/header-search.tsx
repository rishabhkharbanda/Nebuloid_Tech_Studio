...
function SearchPanel({
  ...
  return (
    <div
      id={`${listId}-panel`}
      className={cn(
        'overflow-hidden border shadow-2xl backdrop-blur-xl',
        isDay
    ...
    >
      <div className={cn('border-b p-3', isDay ? 'border-black/10' : 'border-white/10')}>
        ...
        <div className="relative">
          ...
          <input
            ...
            autoComplete="off"
            className={cn(
              'h-11 w-full rounded-xl border pl-9 pr-9 text-base outline-none transition focus:border-[#d4af37]/45 md:text-sm',
              isDay
      ...
      <div id={`${listId}-results`} role="listbox" className="max-h-[min(60vh,24rem)] overflow-y-auto">
        {!query.trim() ? (
          <p
            className={cn(
              'px-4 py-3 text-sm',
...
export function HeaderSearch({ className }: { className?: string }) {
  ...
  const panelProps: SearchPanelProps = {
    listId,
    isDay,
    query,
    setQuery,
      ...
      ? createPortal(
          <div className="fixed inset-0 z-[120] md:hidden">
            <button
              ...
              onClick={closePanel}
            />
            <div className="relative flex h-full flex-col px-4 pb-[max(1rem,env(safe-area-inset-bottom))] pt-[max(4.5rem,env(safe-area-inset-top))]">
              <SearchPanel
                {...panelProps}
                className="flex max-h-full flex-col rounded-2xl"
              />
            </div>
          </div>,
          document.body,
  ...
  return (
    <div ref={rootRef} className={cn('relative', className)}>
      <button
        ...
        className={cn(
          ...
          panelOpen
            ...
            : isDay
              ? 'border-black/15 text-[#181712] hover:border-[#d4af37]/50 hover:text-[#d4af37]'
      ...
      {panelOpen && !isMobile ? (
        <SearchPanel
          {...panelProps}
          className="absolute right-0 top-[calc(100%+0.65rem)] z-[70] w-[min(calc(100vw-3rem),22rem)] rounded-2xl sm:w-[22rem]"
        />
      ) : null}
