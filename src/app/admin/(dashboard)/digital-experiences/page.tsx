      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-sm text-[#6b7280]">
            Manage listing cards and full case-study detail pages (gallery, impact, tech stack).
          </p>
        </div>
        <Link
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {cards.length === 0 ? (
          <p className="text-sm text-[#6b7280]">
            No CMS experiences yet. Static digital projects remain on the public site until you add
            content here.
          </p>
        ) : (
          cards.map((card) => (
              ...
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="text-xs uppercase tracking-[0.14em] text-[#6b7280]">
                    #{card.displayOrder} · {card.status} · {card.enabled ? 'Listed' : 'Hidden'}
                  </p>
                  <h3 className="mt-2 text-lg font-semibold">{card.title}</h3>
                </div>
              </div>
              <p className="mt-2 line-clamp-3 text-sm text-[#6b7280]">
                {card.overview || card.shortDescription}
              </p>
              <p className="mt-3 text-xs text-[#6b7280]">
                Gallery: {card.gallery?.length ?? 0} · Impact: {card.impact?.length ?? 0}
              </p>
            </Link>
          ))
        )}
